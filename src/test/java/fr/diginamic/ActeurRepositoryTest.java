...
public class ActeurRepositoryTest {
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery("SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.realisateurs rea WHERE rea.identite = 'Ridley Scott' AND (f.annee BETWEEN 2010 AND 2020)", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
